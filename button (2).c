...
//    #warning: "Unimplemented function: void init_button_interrupts(int *button_event_addr, int *button_num_addr) -- You must configure GPIO to detect interrupts" // delete warning after implementing
    ...
    //GPIO_PORTE_IM_R &=
    GPIO_PORTE_IM_R &= 0b00000000;
    // 2) Set pins 0-3 to use edge sensing
    //GPIO_PORTE_IS_R &=
    GPIO_PORTE_IS_R &= 0b00000000;
...
//	#warning "Unimplemented function: uint8_t button_getButton(void)"	// delete warning after implementing
    ...
    if((GPIO_PORTE_DATA_R & 0b00000001) == 0b00000000){
        //buttonState = DONE;
        return 1; //sets button_num_ptr to 1 if button 1 is pressed
    }
    else if((GPIO_PORTE_DATA_R & 0b00000010) == 0b00000000){
        //buttonState = DONE;
        return 2; //sets button_num_ptr to 2 if button 2 is pressed
    }
    else if((GPIO_PORTE_DATA_R & 0b00000100) == 0b00000000){
        //buttonState = DONE;
        return 3; //sets button_num_ptr to 3 if button 3 is pressed
    }
    else if((GPIO_PORTE_DATA_R & 0b00001000) == 0b00000000){
        //buttonState = DONE;
        return 4; //sets button_num_ptr to 4 if button 4 is pressed
    }
    else {        //sets button_num_ptr to 0 if no button is pressed
        //buttonState = NOTDONE;
        return 0;
    }
}
